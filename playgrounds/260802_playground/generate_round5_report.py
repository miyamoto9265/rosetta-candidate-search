#!/usr/bin/env python3
"""Round5 short-abbrev no_match reduction report (vs round4 and baseline)."""
...
def main() -> int:
    ...
    p.add_argument("--before", default="round4_abbrev")
    p.add_argument("--after", default="round5_abbrev")
    ...
    showcase = [
        "Thal", "PreS", "PeRh", "vHipp", "dcHPC", "PH", "SOA", "SNpc",
        "PiCo", "mZI", "IntA", "PPRF", "DAO", "ARN", "PaF", "VLPO",
        "DH", "ADn", "HP", "MCPO", "CPr", "BNC", "SGI",
    ]
    ...
    before_label = args.before.replace("_abbrev", "").replace("round", "r")
    after_label = args.after.replace("_abbrev", "").replace("round", "r")
    out = HERE / "runs" / args.after / "round5_improve_report.html"
    doc = f"""<!doctype html>
<html lang="ja">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Round5 improve report — {esc(args.before)} → {esc(args.after)}</title>
<style>
...
<body>
<header>
  <h1>Round5 精度改善レポート（短い略語）</h1>
  <p>
    round4 の短い no_match 略語を対象に、fullname 既知＋文献確認済みの選択的 abbrev / alias
    ルールを追加した結果です。比較: <code>{esc(args.before)}</code> → <code>{esc(args.after)}</code>
    （判定は Flash×3 固定。エンジン v0.8.5）。
  </p>
</header>
<main>
<section>
  ...
  <div class="cards">
    <div class="card"><div class="card-title">no_match</div>
      ...
      <div class="card-sub">{esc(before_label)} {b_all.get('no_match',0)} → {delta(b_all.get('no_match',0), a_all.get('no_match',0))}
    ...
    <div class="card"><div class="card-title">aligned</div>
      ...
      <div class="card-sub">{esc(before_label)} {b_all.get('aligned',0)} → {delta(b_all.get('aligned',0), a_all.get('aligned',0))}
    ...
    <div class="card"><div class="card-title">wrong</div>
      ...
      <div class="card-sub">{esc(before_label)} {b_all.get('wrong',0)} → {delta(b_all.get('wrong',0), a_all.get('wrong',0))}
        / baseline {base_all.get('wrong',0)}</div></div>
    <div class="card"><div class="card-title">{esc(before_label)}→{esc(after_label)} 改善/悪化</div>
      <div class="card-value">{len(imp)} / {len(reg)}</div>
      <div class="card-sub">baseline→r5: {len(imp_b)} / {len(reg_b)}</div></div>
  </div>
  <h3>全体ラベル（{esc(args.before)} → {esc(args.after)}）</h3>
  <div class="table-wrap"><table>
    <thead><tr><th>label</th><th class="num">{esc(args.before)}</th><th class="num">{esc(args.after)}</th><th class="num">Δ</th></tr></thead>
    <tbody>{metric_rows(b_all, a_all)}</tbody>
  </table></div>
  <h3>abbrev のみ</h3>
  <div class="table-wrap"><table>
    <thead><tr><th>label</th><th class="num">{esc(args.before)}</th><th class="num">{esc(args.after)}</th><th class="num">Δ</th></tr></thead>
    <tbody>{metric_rows(b_ab, a_ab)}</tbody>
  </table></div>
</section>
...
<section>
  <h2>Round5 で入れた変更</h2>
  <ol class="tight note">
    <li><b>選択的 abbrev ルール</b> — Thal/PreS/PeRh/vHipp/dcHPC/PH/SOA/SNpc/ARN/PaF など、fullname または文献で用法が明確な短い略語を追加（~130 件）。</li>
    <li><b>abbrev-only 文献確認</b> — PiCo→IRt、mZI→ZI、IntA→InPl、PPRF→pontine RF、DAO→IO など。</li>
    <li><b>alias 補強</b> — posterior hypothalamus→PHN、VLPO→LPA、SOA area→Su3、dorsal horn→spinal horn parent など展開後の解決を保証。</li>
    <li><b>やらないこと</b> — CINs/PoSp/organoid/marker 融合などオントロジー外・細胞種のみはスキップ。曖昧な 2 文字 RF 等は未追加。</li>
  </ol>
</section>

<section>
  <h2>代表例（round4 → round5）</h2>
  <div class="table-wrap"><table>
    <thead><tr><th>query</th><th>round4</th><th>round5</th></tr></thead>
    <tbody>{''.join(show_html) if show_html else '<tr><td colspan=3 class=muted>なし</td></tr>'}</tbody>
  </table></div>
</section>
